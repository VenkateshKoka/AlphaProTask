        ...
        <div className={`${styles.eventsContainer} flex-row`}>
            {currentItems.map(event => (
                <Link key={event.eventId}
                      ...
                      className={`${styles.eventItem}`}> {/* Event Item */}
                    <div className={styles.imageContainer}>
                        <Image src="/microsoft.png" alt="picture of Microsoft logo"
                               height={40}
                               width={40}/>
                    </div>
...
export default function EventsPaginationDuplicate({events}) {
    ...
    return (
        <>
            ...
            <ReactPaginate
                breakLabel="..."
                nextLabel={<span><BsChevronRight/></span>}
                onPageChange={handlePageClick}
                pageRangeDisplayed={2}
                marginPagesDisplayed={2}
                pageCount={pageCount}
                previousLabel={<span><BsChevronLeft/></span>}
